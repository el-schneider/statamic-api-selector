<template>
  <div>
    ...
    >
      ...
      <template #option="{ thumbnailUrl, label }">
        <span class="flex items-center">
          <img v-if="!!thumbnailUrl" class="w-12 h-12" :src="thumbnailUrl" />
    ...
    <div class="text-xs text-gray-600 leading-tight flex">
      <button
        class="text-blue underline whitespace-nowrap mt-2 hover:text-blue-800 ml-auto"
        @click="clearCache"
      >
        Clear Cache
      </button>
...
export default {
  ...
  methods: {
    ...
    clearCache() {
      this.$axios
        .post("/cp/api-selector/clear-cache", {
          cacheKey: this.meta.cacheKey,
        })
        .then((response) => {
          // handle success
